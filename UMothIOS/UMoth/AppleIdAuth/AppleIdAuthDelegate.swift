...
internal final class AppleIdAuthDelegate : NSObject, ASAuthorizationControllerDelegate {
    ...
    private func wrapAppleIdCredential(credential: ASAuthorizationAppleIDCredential) -> AppleIdCredentialWrapper {
        ...
        return AppleIdCredentialWrapper(
            ...
            authorizationCode: allocateNativeBuffer(from: credential.authorizationCode),
            authorizationCodeSize: UInt32(credential.authorizationCode?.count ?? 0),
            identityToken: allocateNativeBuffer(from: credential.identityToken),
            identityTokenSize: UInt32(credential.identityToken?.count ?? 0),
